...
**Boundary with mise:** mise (`mise/.config/mise/config.toml`) owns language runtimes (`go`, `java`, `lua`, `node`, `python`, `neovim`) and most CLI tools (`tmux`, `zoxide`, `oh-my-posh`, `bat`, `fzf`, `pandoc`, `sqlite`, `fastfetch`, `eza`, `lazygit`, `claude`, `codex`, `docker-cli`, `docker-compose`, `tree-sitter`, `gh`). Python is intentionally also installed here so boot and non-interactive scripts have `/usr/bin/python3`; mise remains the interactive development runtime. `ripgrep`, `fd`, `mkcert`, and `jq` are OS-managed. No other mise tool belongs in these lists. `php`/`composer` and `tmuxinator` remain normal package entries (official repo / Brewfile).
