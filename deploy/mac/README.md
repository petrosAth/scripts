...
- **CLI tools** — `tmux`, `zoxide`, `oh-my-posh`, `bat`, `fzf`, `pandoc`, `sqlite`, `fastfetch`, `eza`, `lazygit`, `delta` (Git's pager, via `core.pager`), `claude`, `codex`, `tree-sitter`, `gh`, `tmuxinator` (a `gem:` backend entry on mise's `ruby`), `git-surgeon` (a `cargo:` backend entry on mise's `rust`). `ripgrep`, `fd`, `mkcert`, and `jq` are Brewfile formulae instead.
